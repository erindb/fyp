...
hand_annotated_vs_synset_matching = rbind(
  d %>%
    ...
    mutate(matching.method='wordnet synsets on verbs'),
  df %>%
    group_by(document, chain, clozeIndex, condition) %>% 
    summarise(prop.match = mean(vpmatch=='yes')) %>%
    mutate(matching.method='subjective human judgement'))
...
df %>% group_by(condition) %>%
  ...
  ggplot(aes(x=condition, y=prop.match.orig)) +
  geom_bar(stat='identity')
```

histograms and entropy
link in weekly update with additional figures
