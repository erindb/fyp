...
However, hand-coding whether the gist of the response was similar to the gist of the original event, people do a lot better. Still, only about 25% of the stimuli were recovered by any of the participants, and on those conditions, only about 65% of participants chose that response.

```{r}
df = read.csv('experiment3-annotated.csv')

df %>% filter(condition=='original') %>% group_by(condition) %>%
  summarise(prop.match.orig = mean(vpmatch=='yes', na.rm=T)) %>%
  ggplot(aes(x=1, y=prop.match.orig))

## proportion of matching original (manually tagged)
prop.match.orig = df %>% group_by(document, chain, clozeIndex, condition) %>%
  summarise(prop.match.orig = mean(vpmatch=='yes', na.rm=T)) %>%
  as.data.frame %>% group_by(condition) %>%
  summarise(prop.match.orig = mean(prop.match.orig))

## proportion of tasks where at least one person got it (manually tagged)
prop.anyone.got.it = df %>% group_by(document, chain, clozeIndex, condition) %>%
  summarise(match.orig = mean(vpmatch=='yes', na.rm=T)>0) %>%
  as.data.frame %>% group_by(condition) %>%
  summarise(prop.match.orig = mean(match.orig))

## proportion of matching original (manually tagged) given success
match.orig.given.success = df %>% group_by(document, chain, clozeIndex, condition) %>%
  summarise(match.orig = mean(vpmatch=='yes', na.rm=T)>0,
            prop.match.orig = mean(vpmatch=='yes', na.rm=T)) %>%
  filter(match.orig) %>%
  as.data.frame %>% group_by(condition) %>%
  summarise(prop.match.orig = mean(prop.match.orig))
```
