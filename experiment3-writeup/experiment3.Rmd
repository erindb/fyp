...
ggsave('human.cloze.orig.png', width=5, height=3)
```
...
ggsave('human.agreement.hist.auto.png', width=8, height=10)
```
...
df %>% filter(condition=='original') %>%
  ...
  # theme(axis.text.x = element_blank())
  theme(axis.text.x = element_text(angle=60, hjust=1))
ggsave('human.agreement.hist.manual.png', width=8, height=10)
```
...
agreement.df %>% filter(condition=='original') %>% group_by(method) %>%
  ...
  ylim(0, 1)
ggsave('human.agreement.orig.png', width=5, height=3)
```
...
hand_annotated_vs_synset_matching %>%
  ...
  ylim(0, 1)
ggsave('human.cloze.all.png', width=5, height=3)
```
...
agreement.df %>%
  ...
  geom_errorbar(aes(x=condition, ymin=low, ymax=high), width=0.01) +
  ylim(0, 1)
ggsave('human.agreement.all.png', width=5, height=3)
```
