...
## Script induction
@rudinger_rachel_learning_2015 used a representation of scripts from @chambers_unsupervised_2008 and @jans_skip_2012.
...
@rudinger_rachel_learning_2015 ran several script induction techniques on a relatively small dataset of stories about restaurants (pulled from the blog ["Dinners from Hell"](http://www.dinnersfromhell.com/)) and have made [their code](https://github.com/rudinger/nachos) publicly available online.
...
* **Human narrative cloze:**  In much of the literature on script induction, the narrative cloze task is used to evaluate performance. In this task a subset of documents is held out from training, and at test the model is given some of the events that occurred and asked to guess another event that occurred at some time relative to the known events. The general script knowledge is used to try to fill in the missing information. {{someone said humans can't actually do this probably, but both the model and humans can make guesses}} We will compare the responses of the model to responses that humans give, determining the amount of overlap.
...
* extend to other corpora
* extend to include categorical or causal information explicitly
...
Another representation for human theories, a superset of both scripts and causal Bayes nets, are probabilistic programs. Probabilistic programs . Probabilistic programs can represent complex relationshipts between events and causation, but their complexity makes them difficult to learn automatially without sufficient constraints.
    ...
    Previous work has used survey questions on Amazon's Mechanical Turk to validate the cohesion of events.
...
* argument coreference across events in a script
    ...
    e.g. "At a restaurant, someone serves something and then someone eats something. How likely is it that the same person does both the eating and the serving?" or "How likely is it that the same thing is both served and eaten?"
